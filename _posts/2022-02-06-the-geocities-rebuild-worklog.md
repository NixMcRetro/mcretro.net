...
\[text\] sudo chown www-data:www-data -R \* sudo find . -type d -exec chmod 755 {} \\; sudo find . -type f -exec chmod 644 {} \\; \[/text\]
...
I am very glad to see that the symlinks survived. Now it's on to trying to determine the best way to structure the drive. All the links in the table on [Area51](http://geocities.mcretro.net/neighborhoods/#Area51) should now be active - at least until I try copying more data to it.
...
\[text\] sudo mount /dev/sdb2 /var/www/html/geocities \[/text\]

I had been considering mounting the drive as read only with:

\[text\] sudo mount -oro,noload /dev/sdb2 /var/www/html/geocities \[/text\]
...
\[text\] sudo bindfs -o create-with-perms=0644:a+X,force-group=www-data,force-user=www-data,create-for-user=www-data,create-for-group=www-data /media/geocities_ext/neighborhoods /var/www/html/geocities \[/text\]

It looks like there's a faster way to change permissions that I wasn't aware of until today too.

\[text\] cd /var/www sudo chown www-data:www-data -R \* sudo chmod -R a=r,a+X,u+w . \[/text\]

That last line is a replacement for:

\[text\] sudo find . -type d -exec chmod 755 {} \\; sudo find . -type f -exec chmod 644 {} \\; \[/text\]

and then to unmount the partition and play with the data we just run:

\[text\] sudo umount /var/www/html/geocities \[/text\]
...
\[text\] 755 (drwxr-xr-x) \[/text\]

and a file to:

\[text\] 644 (-rwxr--r--) \[/text\]
...
\[text\] ln -s Station station \[/text\]
...
\[text\] sudo a2enmod speling \[/text\]

We've placed the following in VirtualHost configuration file inside Directory tags: \[text\] # Case-insensitive CheckSpelling on CheckCaseOnly on \[/text\]
...
A redirection to **[https://geocities.mcretro.net/CapeCanaveral/](https://geocities.mcretro.net/CapeCanaveral/)** ... I guess I won't be needing those symlinks after all thanks to Apache's **mod_speling**.
...
For now I'm going to get the [neighborhood list](http://geocities.mcretro.net/neighborhoods/) tabled up and cross checked between a few sources and the [WayBack Machine](https://web.archive.org/web/19990203120656/http://www4.geocities.com/neighborhoods/).
...
\[text\] Substitute 's|<script language="JavaScript" src="http://us.js2.yimg.com/us.js.yimg.com/lib/smb/js/hosting/cp/js_source/geov2_001.js"></script>|<img src="http://geocities.mcretro.net/pictures/pickle.gif">|i' \[/text\]
...
So for now there is a spinning pickle on all the pages that are up, until a header is added. Definitely check out CapeCanaveral and Area51 to see if you can find any without spinning pickles. What a time to be alive. I better get back to creating the [neighborhood pages](https://geocities.mcretro.net/neighborhoods.php).
...
\[text\] sudo apt update sudo apt install npm sudo npm install -g sitemap-generator-cli \[/text\]
...
\[text\] sitemap-generator http://geocities.mcretro.net --verbose --max-depth 4 \[/text\]

![](/assets/images/2022/img_0895.jpg)

After waiting what felt like a few hours, move the sitemap into the appropriate folder:

\[text\] sudo mv sitemap.xml /var/www/html/geocities/ \[/text\]

![](/assets/images/2022/img_0894.jpg)

We now have a [sitemap](https://geocities.mcretro.net/sitemap.xml). Next, we need to figure out how to best implement the teleport (Jump™) feature to utilise the sitemap.
...
You can try it [here](https://geocities.mcretro.net/jump/) to beam to a random GeoCities website. We still have only Area51 and CapeCanaveral active. Great for fans of X-Files fanfic, not so great for anyone else.
...
The above shows issues when using BODY as a place to inject when FRAMESET is present. Substituting the \[text\]</BODY>\[/text\] results in no nav buttons showing because [FRAMESET is used in place of BODY tags](https://www.w3.org/TR/html4/present/frames.html). While I could inject code into FRAMESET as well, I think it would cause more issues depending on if the FRAMESET contained body tags such as the above sample site code.
...
\[text\] sudo nano /etc/apache2/sites-available/geocities.conf \[/text\]

and inserted the following:

\[text\] # Insert Home, Jump™ and Report Icons - Works without breaking too much, doesn't show on sites without a closing body tag </BODY>. Substitute 's|</body>|<BR><p align="right"><a HREF="/" target="_top" rel="noopener"><IMG SRC="/pictures/home.gif" ALT="Home"></A> <a HREF="/jump" target="_top" rel="noopener"><IMG SRC="/pictures/jump.gif" ALT="Jump!"></A> <a HREF="http://mcretro.net/report" target="_blank" rel="noopener"><IMG SRC="/pictures/report.gif" ALT="Report"</A></p></body>|i' \[/text\]

We can now jump from most pages - including directory listings (i.e. pages without an index).

In other news, I've mostly finished the [neighborhood listing](https://geocities.mcretro.net/neighborhoods/). This means if I copy all the files across they will now load up well enough. I'm still working on directory structure though.
...
\[text\] DocumentRoot /var/www/html/geocities <Directory /var/www/html/geocities> Options Indexes MultiViews FollowSymLinks AllowOverride None Require all granted </Directory>
...
RewriteCond "/var/www/html/External/Core%{REQUEST_URI}" -f \[OR\] RewriteCond "/var/www/html/External/Core%{REQUEST_URI}" -d RewriteRule ^/?(.\*)$ /var/www/html/External/Core/$1 \[L\] \[/text\]
...
\[text\] "mod_speling and mod_rewrite apply their rules during the same phase. If rewrite touches a URL it generally won't pass the url on to mod_speling." \[/text\]
...
\[text\] sudo ln -s Athens athens \[/text\]
...
\[text\] # Rewrites for GeoCities (self-made) files. Allows mod_speling to function. # mod_rewrite does not allow mod_speling to modify / ignore case. # This is fine on my created files as everything is lowercase. RewriteEngine on RewriteCond "/var/www/html/geocities%{REQUEST_URI}" -f \[OR\] RewriteCond "/var/www/html/geocities%{REQUEST_URI}" -d RewriteRule ^/?(.\*)$ /var/www/html/geocities/$1 \[L\] \[/text\]
...
\[text\] sudo ln -s /var/www/html/External/Core/\* /var/www/html/External/YahooIDs \[/text\]
...
\[text\] sudo ln -sf /var/www/html/External/Core/\* /var/www/html/External/YahooIDs \[/text\]

\[text\] -f Force existing destination pathnames to be removed to allow the link. \[/text\]
...
**UPDATE 1:** These are some other good test cases while trying to get the merging to work as some were present in lowercase and some in uppercase. Ultimately I'll need to merge all the case variants at some stage.

**Exists** [http://geocities.mcretro.net/capecanaveral/1020/line/](http://geocities.mcretro.net/capecanaveral/1020/line/)

**Doesn't Exist** [http://geocities.mcretro.net/CapeCanaveral/1020/line/](http://geocities.mcretro.net/CapeCanaveral/1020/line/)

Great links for testing this out: [http://geocities.mcretro.net/CapeCanaveral/1020/line/marsh.gif](http://geocities.mcretro.net/CapeCanaveral/1020/line/marsh.gif) [http://geocities.mcretro.net/capecanaveral/1020/line/marsh.gif](http://geocities.mcretro.net/capecanaveral/1020/line/marsh.gif) [http://geocities.mcretro.net/CapeCanaveral/1020/index.html](http://geocities.mcretro.net/CapeCanaveral/1020/index.html) [http://geocities.mcretro.net/capecanaveral/1020/index.html](http://geocities.mcretro.net/capecanaveral/1020/index.html)

Does mod_spelling work it out? If yes, mod_spelling is working well to merge the mush of uppercase and lowercase.

**UPDATE 2:** Copied all the neighborhoods (core) across and a few usernames (yahooids) - it looks like it might be working. I'll rebuild the sitemap.xml file tonight. It should wind up being pretty big. Then jump should teleport across any page that is available!
...
[Jump™](http://geocities.mcretro.net/jump) is working, however the server does not like having to parse a file that is 15MB everytime the link is clicked. There's over 280,000 links indexed. Yikes! That's before I add 100,000+ user folders... That's a lot of files.
...
\[text\] <!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN"> <HTML> <HEAD> <TITLE>Index of /CapeCanaveral/6276/wreck</TITLE> </HEAD> <BODY> <H1>Index of /CapeCanaveral/6276/wreck</H1> \[/text\]

From that we can steal the first part - four lines worth:

\[text\] <!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN"> <HTML> <HEAD> <TITLE>Index of \[/text\]
...
\[text\] grep --include=\\\*.{htm,html} -rnw '/path/to/somewhere/' -e "pattern" \[/text\]
...
\[text\] pcregrep --buffer-size=2M --include=\\index.{htm,html} -rilM "(?s)<\\!DOCTYPE HTML PUBLIC \\"-//W3C//DTD HTML 3.2 Final//EN\\">.\*(?s)<HTML>.\*<HEAD>.\*(?s)<TITLE>Index of " '.' 2>&1 |tee ~/Desktop/blankindex.txt \[/text\]
...
\[text\] pcregrep --buffer-size=2M --include=\\index.{htm,html} -rilM "(?s)<HTML>.\*<HEAD>" '.' 2>&1 |tee ~/Desktop/fullindex.txt \[/text\]
...
\[text\] xargs -n 1 ls -v < ~/Desktop/blankindex.txt \[/text\]
...
\[text\] xargs -n 1 rm -v < ~/Desktop/blankindex.txt \[/text\]

It worked. Next stop, clean out some more empty files, folders and .DS_Stores:

\[text\] find . -name .DS_Store -delete find . -empty -type f -delete find . -empty -type d -delete \[/text\]
...
\[text\] find . | awk '{print length($0), $0}' | sort -n | tail -n 10000 \[/text\]
...
\[text\]/Athens/olympus/\[/text\] is not redirected to \[text\]/Athens/Olympus/\[/text\] as \[text\]/Athens/olympus/\[/text\] exists as a symlink. This causes issues for mod_speling as it can't resolve issues like this as it sees two olympus folders, even though they are one and the same.
...
301,000 pages are now in the [sitemap.xml](https://geocities.mcretro.net/sitemap.xml) split over seven files. That's with a depth flag of 4. I'm scared to think how long it would take if we did unlimited depth following every link found. I should try it one decade. Anyway, now that the sitemap is working, it's time to fix [Jump™!](https://geocities.mcretro.net/jump)
...
**The pictures are (mostly) merged** In the image above we can see clipart tiled backgrounds making an appearance which makes text a lot easier on the eyes. I've been thinking about implementing a calibration page to help get the best 800x600 experience on modern browsers as some of the tiling tends to, well tile I guess, vertically making the page look like sloppy construction.

![](/assets/images/2022/img_0903.jpg)

There's now a GeoCities [favicon.ico](https://geocities.mcretro.net/favicon.ico) in the title bar. It looks like the favicon standard wasn't around until the late 1990s when GeoCities rebranded, just prior to Yahoo! takeover.

![](/assets/images/2022/img_0890.jpg)

Since just before 7am we have been rebuilding the [sitemap.xml](https://geocities.mcretro.net/sitemap.xml) to reflect the lack of empty folders and blank index.html files. This will likely take the rest of the day.

After this has completed, I've allocated some time to spend browsing the GeoCities via [Jump™](http://geocities.mcretro.net/jump/). I'll live stream this browsing session over one of my [YouTube channels](/channels/). This will be done with the MiSTer which feels like a 486 running at 33MHz. Armed with Windows 98 and Internet Explorer 5.0, I'll dial into the world wide web (WWW) through my two Banksia SP56 modems at ~28.8k. I've got an [MT32-pi](https://github.com/dwhinham/mt32-pi) hooked up so I'll be rocking out to any MIDIs I come across. Stay tuned! :city_sunrise:
...
Try it out at [http://geocities.mcretro.net](http://geocities.mcretro.net) if you haven't already.
...
Once clicked you end up with the very easy to remember: [http://geocitiesllczuf44da2nj45jn3fntdjpw27ercfbhkrw3mnegti7pid.onion](http://geocitiesllczuf44da2nj45jn3fntdjpw27ercfbhkrw3mnegti7pid.onion)
...
[Grab the Tor Browser](https://www.torproject.org/download/) and have a tinker with the technology, it's quite interesting to setup. Then stop by [http://geocitiesllczuf44da2nj45jn3fntdjpw27ercfbhkrw3mnegti7pid.onion](http://geocitiesllczuf44da2nj45jn3fntdjpw27ercfbhkrw3mnegti7pid.onion)
...
If you visit [https://geocities.mcretro.net](https://geocities.mcretro.net/) in the Tor Browser, please note the activation of the location bar. [Onion-Location](https://support.torproject.org/onionservices/onion-location/) activates. This will not work if visiting on clearnet non-ssl site, [http://geocities.mcretro.net](http://geocities.mcretro.net/). This is [by design](https://community.torproject.org/onion-services/advanced/onion-location/) according to The Tor Project.
...
\[text\] sudo tail -f /var/log/apache2/other_vhosts_access.log sudo tail -f /var/log/apache2/error.log sudo tail -f /var/log/apache2/access.log \[/text\]

![](/assets/images/2022/img_0842.jpg)

They even show up maroon! How did I manage this, I had renamed the external drive from "External" to "geocities" to help differentiate it from another project I had activated in the past week or so - [The Assembler Games Archive](https://assembler.mcretro.net). All my old symlinks died as a result.
...
Best Regards, INSC, Tsinghua University \[/text\]

I thought, that’s interesting. Naturally, I was curious as to what I had misconfigured. So I visited my sitemap domain as a test [http://retrojunkie.net/server-status](http://retrojunkie.net/server-status) and was presented with this:

![](/assets/images/2022/img_0817.jpg)

Works as expected on the clearnet, external IPs shouldn’t be able to see it. But onion domains use localhost (127.0.0.1) to do some funky routing. Let’s try the darknet version of the same site at: [http://retro56qdosu3zuisr5tiuh7ihpn6owzpwjwvs7djlweiqb3mcspahyd.onion/server-status](http://retro56qdosu3zuisr5tiuh7ihpn6owzpwjwvs7djlweiqb3mcspahyd.onion/server-status)
...
\[text\] sudo a2dismod status sudo a2dismod info sudo apache2ctl configtest sudo systemctl restart apache2.service \[/text\]
...
**Sources:** [https://forum.acms.org.au/t/geocities-rebuild-for-older-browsers/83](https://forum.acms.org.au/t/geocities-rebuild-for-older-browsers/83) [https://archive.fo/eRyuQ](https://archive.fo/eRyuQ)
